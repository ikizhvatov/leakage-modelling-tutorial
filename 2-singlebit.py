'''
Playing around with linear regression - 2

This example is similar to example 1, but the data is drawn from real
 measurements of AES encryption on ATmega. We look at a single bit of the
 1 round S-box output and buid a model for the leakage using linear
 regression (with ordinary least squares). A more elaborate OLS implementation
 is used, namely the one which is suitable for multivariate OLS.
 Aditionaly, we plot histograms to describe the leakge in the Gaussian model.

Started by Ilya on 2013-05-15
...
# Parameter: bit of the S-Box output to look at
bit = 0
...
# load AES S-Box stored in the file
